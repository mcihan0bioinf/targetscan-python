...
def _region_span_length(region_str: str) -> int:
    """Total length of a (possibly multi-block) "chrom:start-end; ..." string."""
    total = 0
    for block in region_str.split(";"):
        block = block.strip()
        if not block:
            continue
        _chrom, coords = block.rsplit(":", 1)
        start, end = coords.split("-")
        total += int(end) - int(start) + 1
    return total


def read_liftover_report(report_path: str) -> Dict[str, dict]:
    """Read a liftover report TSV (as written by ``write_report``/the
    ``--gff`` runner) back into ``{gene_id: row_dict}``. Note this does
    *not* include sequences (those aren't persisted to the report, to
    keep it small for tens of thousands of transcripts) -- use
    ``apply_liftover_report_to_utr_file`` for applying it at scale."""
    rows = {}
...
def apply_liftover_report_to_utr_file(
    report_path: str, utr_file: str, out_path: str, applied_report_path: str
) -> None:
    """Streaming variant of ``apply_liftover_to_utr_file`` for full-scale
    use: works directly off a persisted report TSV (tens of thousands of
    rows) against a multi-GB UTR_Sequences.txt, without needing the
    original sequences held in memory or re-fetched from Ensembl.

    Since the report doesn't carry sequences, "ok" rows are verified by
    *length* only here: does the existing alignment row's ungapped length
    match the total length of the lifted hg19 region(s)? This is weaker
    than the full sequence-identity check ``apply_liftover_to_utr_file``
    does (used for the small/demo-scale path), but for "ok"-tagged rows
    (already Ensembl-verified hg19==hg38 at the genomic level) a length
    match is strong corroborating evidence that the file's row is the
    same region, without re-fetching ~28,000 sequences.
    """
    rows = read_liftover_report(report_path)
    applied_log = []
    ...
    with open(utr_file) as fh, open(out_path, "w") as out:
        for line in fh:
            ...
            if species_id == HUMAN_SPECIES_ID and gene_id in rows:
                row = rows[gene_id]
                if row["tag"] == "ok":
                    expected_length = _region_span_length(row["hg19_region"])
                    existing_length = len(seq.replace("-", ""))
                    if existing_length == expected_length:
                        applied_log.append((gene_id, "applied (length-verified, coordinates re-anchored)"))
                    else:
                        applied_log.append(
                            (
                                gene_id,
                                f"NOT applied: alignment row length {existing_length} != "
                                f"lifted region length {expected_length}",
                            )
                        )
                else:
                    applied_log.append((gene_id, f"NOT applied: tag={row['tag']}"))
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    if argv[0] == "--apply-from-report":
        report_path, utr_file, utr_out, applied_out = argv[1], argv[2], argv[3], argv[4]
        print(f"Applying {report_path} to {utr_file}...", file=sys.stderr)
        apply_liftover_report_to_utr_file(report_path, utr_file, utr_out, applied_out)
